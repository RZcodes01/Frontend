  ...
  if (loading) {
    return (
      <div className="flex flex-col h-96 items-center justify-center gap-4">
        ...
        <p className="text-blue-900 font-bold animate-pulse">Synchronizing Mentor Data...</p>
      </div>
    );
  }
    ...
    <div className="space-y-12 pb-20">
      ...
      <section>
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-3xl font-black text-[#1e3a5f] tracking-tight">RESUME VERIFICATION</h2>
            <p className="text-blue-400 font-medium italic">Review and approve new mentor applications</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {pendingMentors.map((mentor, index) => (
            <div key={mentor._id || `pending-${index}`} className="bg-white rounded-3xl border border-blue-100 shadow-sm overflow-hidden flex flex-col">
              <div className="p-6 flex-1">
                ...
                <h3 className="text-xl font-bold text-[#1e3a5f] capitalize">{mentor.name}</h3>
                <div className="text-sm text-blue-400 flex items-center gap-2 mt-1">
                  <Mail size={14} /> {mentor.email}
                </div>
                <div className="mt-6 p-4 bg-blue-50 rounded-2xl border border-blue-100">
                  <a href={mentor.resumeUrl} target="_blank" rel="noreferrer" className="flex items-center justify-between text-[#1e3a5f] font-bold text-sm hover:text-blue-600 transition-colors">
      ...
      <section>
        <div className="bg-[#1e3a5f] rounded-3xl p-8 mb-8 flex items-center justify-between text-white relative overflow-hidden">
          {/* Amber accent bar matching Hero sidebar */}
          <div className="absolute top-0 left-0 w-1.5 h-full bg-amber-400" />
          <h2 className="text-2xl font-black tracking-tight flex items-center gap-3 ml-4">
            <ShieldCheck className="text-amber-400" size={28} /> ACTIVE MENTORS HUB
          </h2>
          <div className="text-right">
            <span className="text-4xl font-black text-amber-400">{activeMentors.length}</span>
            <p className="text-[10px] font-bold text-blue-300 uppercase">Total Mentors</p>
          </div>
        </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-blue-50">
              {activeMentors.map((m, index) => (
                <tr key={m._id || `active-${index}`} className="hover:bg-blue-50/50 transition-colors">
                  <td className="p-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="font-bold text-[#1e3a5f] capitalize">{m.name}</div>
                    </div>
                  </td>
